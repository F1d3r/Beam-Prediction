    ...
    def __getitem__(self, index):
        ...
        for i in range(self.seq_len):
            if self.augment['camera'] == 0:
                if 'scenario31' in add_fronts[i] or 'scenario32' in add_fronts[i]:
                    if self.augment['camera'] == 0:  # segmentation added to non augmented data
                        if self.add_mask:
                            imgs = np.array(
                                Image.open(self.root + add_fronts[i][:30] + 
                                           '_mask' + add_fronts[i][30:]).resize((256, 256)))
                        else:
                            imgs = np.array(Image.open(self.root + add_fronts[i]).resize((256, 256)))
                            if self.add_seg:
                                seg = np.array(
                                    Image.open(self.root + add_fronts[i][:30] + 
                                               '_seg' + add_fronts[i][30:]).resize((256, 256)))
                                a = seg[..., 2]
                                a = a[:, :, np.newaxis]
                    ...
                    if self.add_mask:
                        ...
                        imgs = np.array(Image.open(self.root + add_fronts[i][:30]+
                                                   '_raw'+add_fronts[i][30:]).resize((256, 256)))
            else: #1
                imgs = np.array(Image.open(self.root+add_fronts[i]).resize((256,256)))
                #Read image data 256*256*3 (enhanced only)
            #radar data
            radar_ang1 = np.load(self.root + add_radars[i])
            #Read radar data 256*256(angle)
            ...
            if self.add_velocity:
                ...
                # @2*256*256(Spell ang and vel together)
            else:
