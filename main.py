...
from tqdm import tqdm	# Monitoring the traning progress.
...
parser.add_argument('--id', type=str, default='test_cui', 
...
# /ibex/scratch/tiany0c/log
parser.add_argument('--add_velocity', type = int, default=1, 
                    help='concatenate velocity map with angle map')
parser.add_argument('--add_mask', type=int, default=0, 
                    help='add mask to the camera data') 
parser.add_argument('--enhanced', type=int, default=1, 
                    help='use enhanced camera data') 
parser.add_argument('--filtered', type=int, default=0, 
                    help='use filtered lidar data') 
parser.add_argument('--loss', type=str, default='focal', 
                    help='crossentropy or focal loss') 
parser.add_argument('--scheduler', type=int, default=1, 
                    help='use scheduler to control the learning rate') #
parser.add_argument('--load_previous_best', type=int, default=0, 
                    help='load previous best pretrained model ') 
parser.add_argument('--temp_coef', type=int, default=1, 
                    help='apply temperature coefficience on the target') 
parser.add_argument('--train_adapt_together', type=int, default=1, 
                    help='combine train and adaptation dataset together') 
parser.add_argument('--finetune', type=int, default=0, 
                    help='first train on development set and finetune on 31-34 set')
parser.add_argument('--Test', type=int, default=0, 
                    help='Test')
parser.add_argument('--augmentation', type=int, default=1, 
                    help='data augmentation of camera and lidar')
parser.add_argument('--angle_norm', type=int, default=1, 
                    help='normlize the gps loc with unit, angle can be obtained')
parser.add_argument('--custom_FoV_lidar', type=int, default=1, 
                    help='Custom FoV of lidar')
parser.add_argument('--add_seg', type=int, default=0, 
                    help='add segmentation on 31&32 images')
parser.add_argument('--ema', type=int, default=0, 
                    help='exponential moving average')
parser.add_argument('--flip', type=int, default=0, 
                    help='flip all the data to augmentation')
		...
		if args.finetune:
			if DBA>self.DBAft[-1]:
				...
				torch.save(model.state_dict(), 
                           os.path.join(args.logdir, 'all_finetune_on_' + kw + 'model.pth'))
				torch.save(optimizer.state_dict(), 
                           os.path.join(args.logdir, 'all_finetune_on_' + kw + 'optim.pth'))
			else:
		...
		with torch.no_grad():	
			...
			for s in scenarios:
				...
				if np.sum(beam_scenario_index) > 0:
					curr_acc_s = compute_acc(pred_beam_all[beam_scenario_index], 
                                             gt_beam_all[beam_scenario_index], top_k=[1,2,3])
		...
		for j in range(0, lidar_aug_num + 1):
			for k in range(0, radar_aug_num + 1):
				if i == 0 and j == 0 and k == 0:	# skip the original dataset
					continue
				augmentation_entry = CARLA_Data(root=val_root, root_csv=root_csv, 
                                                config=config, test=False, 
                                                augment={'camera':i, 'lidar':j,'radar':k})
				if augmentation_set == []:
...
if __name__ == '__main__':
	...
	def createDataset(InputFile, OutputFile, Keyword):
		...
		with open(RawFile) as infile, open(CleanedFile, 'w') as outfile:
			...
			for row in reader:
				try:
					# Detect the scenario keyword in the filename of each row.
	...
	if args.finetune and not args.Test:
		...
		# Randomly split the data into training set and validation set by 80/20.
		train_set, val_set = torch.utils.data.random_split(development_set, [train_size, len(development_set) - train_size])
